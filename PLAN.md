...
| **Siber-güvenlik bilen hukukçu danışmanlık** (sahip kararı 2026-06-23) — T&S kapsamına eklenen yeni kavramların hukuki çerçevesi: toplumu öfkeye/saldırganlığa sürükleme + terör örgütü/bölücü propaganda (TR'de TCK suç + 5651). **Kelime-filtresi DEĞİL** → reaktif moderasyon (4B Report→kuyruk→insan) + hukuki eskalasyon | Kurucu + hukuk | Lansman önü (A2 ile birlikte) |
...
| D5 | Bayat `before` cursor en son 50'yi döndürür; `?limit=` controller'da ölü; WS CORS `*` | api | denetim #5/#6/#8 | düşük | ✅ çözüldü (2026-06-20): WS CORS zaten `FRONTEND_URL`'di (bayat kayıt, `*` yok); ölü `limit` param kaldırıldı (sabit `take=50`); bayat cursor `findMessages`+`searchMessages`'te artık `[]` döner (frontend döngü riski kapandı) + test güncellendi |
| D6 | Hesap silme `cancel` endpoint + `me()` 403 ulaşılamaz (delete tüm session'ları revoke eder; gerçek iptal = login) | api | 2B R7 review #1 | düşük | ✅ çözüldü (2026-06-20): ölü `cancelAccountDeletion` endpoint + service metodu + frontend `cancelDeleteAccount()` **kaldırıldı** (yanıltıcı "deaktif oturum istisnası" hiç implemente edilmemişti); `me()` 403 savunma-derinliği olarak **korundu** + yorumlandı; SPRINT_2B_CONTRACT + frontend notu güncellendi. Gerçek iptal = re-login. 743/743 test yeşil |
...
| D10 | `findMessages` `before` cursor'ı `deletedAt` kontrol etmiyor — silinmiş mesaj cursor olarak verilirse `lt` yanlış tarih alır (D5 cursor borcuyla akraba) | api | Sprint 4A B1 test gözlemi / 2026-06-13 | düşük (kötü-niyet sınırlı: kendi kanalın) | ✅ kapandı-analizle (2026-06-20): **gerçek bug değil** — soft-delete `createdAt`'i değiştirmez → silinmiş cursor'ın `lt` sınırı doğru; çapraz-kanal cursor yalnız timestamp okur (içerik sızıntısı yok, sonuç kanal-filtreli). [[D5]] zaten geçersiz cursor'ı `[]` yapıyor. Kanal-scope marjinal cleanliness → over-engineering kaçınıldı |
...
| D12 | Davet linki `/invite/:code` rotası yok — kopyalanan link uygulamada 404; katılım kodla çalışıyor (link kozmetik) | web | Sprint 7A frontend / 2026-06-13 | düşük (UX) | ✅ çözülmüş-bulundu (2026-06-20): `/invite/:code` rotası **MEVCUT** (`router/index.ts` → `InviteAcceptView`, `requiresAuth` + login-redirect path-koruma); `InvitePeopleModal` linki `origin + /invite/:code` üretiyor; view `preview` + `joinByInvite` ile uçtan uca çalışıyor. Bayat kayıt |
...
| D22 | **Responsive/mobil uyum yok** — web SPA desktop-first; ~768px altında app-shell (rail 72px + panel 264px **sabit**, drawer/hamburger yok) daralmıyor → telefonda içerik ~75px, kullanılamaz. Landing + auth responsive; app-shell değil. Flutter mobil ertelendiğinden **web = tek mobil yol** | web | responsive durum tespiti / 2026-06-22 | UX (açık kayıtta yüksek — TR trafiği mobil ağırlık); **demo blokeri DEĞİL** (invite-only → mobil rando app'e giremez, responsive landing'i görür) | ✅ **TAMAMLANDI + CANLI 2026-06-23:** `contracts/SPRINT_RESPONSIVE_CONTRACT.md`. Discord-masaüstü modeli + mobilde (`<768`) **tek birleşik sol-drawer** (rail+sidebar). Tailwind v4 `md/lg/xl`=768/1024/1280 + `@vueuse useBreakpoints`. **A** (✓ `useResponsive`/`useAppShellNav` + app-shell off-canvas drawer/hamburger/backdrop/ESC/scroll-lock + üye paneli ≥1280-inline/`<1280`-overlay; nullable `rightPanelOpen`+`isWide`-türevli `rightPanelVisible` ile geniş-ekran regresyonsuz) · **B** (✓ HomeDashboard grid reflow `cols-1 sm:2 lg:3 +import xl:4`; liste/mesaj panelleri zaten `min-w-0`+truncate) · **C** (✓ KvModal `<768` tam-ekran→türeyen tüm modallar + ServerModal/FriendAddModal/StartChatModal ayrı; UserSettingsView+GuildSettingsView mobilde üst yatay sekme şeridi). **R7-nötr.** **Ertelendi→V2:** ses/video tile responsive · swipe jesti · dokunmatik sürükle-sırala. **Açık ürün sorusu (sahip 2026-06-23):** karşılama/dashboard ekranı kalsın/küçülsün/gitsin mi — responsive'den bağımsız, canlıda görülüp karar verilecek |
...
- **A2 Hukuki/uyum** — deneyimli (siber-güvenlik bilen) görüş → 4B §3 doldur (CSAM akışı + contextSnapshot içerik = şu an STUB), 5651/kanıt-saklama/legal-hold, nihai KVKK + gizlilik metni. **+ Yeni T&S kavramları (sahip 2026-06-23):** toplumu öfkeye sürükleme + terör örgütü/bölücü propaganda (TR'de TCK suç) — kelime-filtresi değil **reaktif moderasyon + hukuki eskalasyon**; çerçeve hukukçuyla netleşir. **Açık beta bunsuz olmaz** (kurul). (`SPRINT_4B_HUKUK_BRIEF.md`)
...
- **A5 Public davet linki (viral; Discord `discord.gg/xxx` muadili) — T&S korumalı (FİKİR, sahip 2026-06-23)** — Sahip, kullanıcıların ortam linklerini dışarıda paylaşıp yeni kişi çekmesini istiyor (haklı — Discord alternatifi için davet linki olmazsa-olmaz, viral büyümenin motoru). **Zaten VAR (D12):** `/invite/:code` rotası + `InvitePeopleModal` `origin + /invite/:code` linki üretiyor + giriş-yapmış kullanıcı için uçtan uca katılım çalışıyor (`requiresAuth` + login-redirect). **Eksik olan tek şey** Discord deneyimini Discord yapan kısım: linke tıklayan **giriş-yapmamış/hesapsız** ziyaretçi için karşılama/önizleme sayfası → kayıt/giriş → otomatik katılım. **Bugün anlamsız:** `REGISTRATION_MODE=invite` (kapalı test) link-tıklayanın kayıt olmasını engelliyor; iki davet kapısı (platform vs ortam) birbirini iptal eder. → **Açık kayıt (G2 flip `open`) ile birlikte açılır, ÖNCE değil.** **T&S kapıları (kurul ilkesi "ulaşımı zorlaştır" — public link en riskli çocuk-güvenliği vektörü; Discord'da `discord.gg` grooming/CSAM dağıtım kanalı):** (1) `adultsOnly` ortamlar public link **üretemez** (yalnız uygulama-içi kod paylaşımı) — yoksa minör keşif yüzeyi açılır; (2) link önizlemesi minöre yetişkin içerik/ortam sızdırmaz (mevcut `GET /invites/:code` minimal-veri deseni korunur); (3) rate-limit + expiry + revoke (expiry/revoke **var**, rate-limit eklenir); (4) opsiyonel: link üretimini belli olgunluk/güven seviyesindeki ortamlara kısıtla. **Teknik iş küçük** (unauth landing + kayıt→join akışı); **asıl iş T&S tasarımı + zamanlama.** **Durum: açık-kayıt/lansman fazı; şimdi build değil** (kapalı testte gereksiz + riski erken davet eder).
