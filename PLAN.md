...
### Track B — Etkinlik Motoru (additive; şema ileri-uyumlu hazır — "yarın" dediğimiz)
- **B1 Tekrarlama motoru** — `recurrence` enum hazır → DAILY/WEEKLY/MONTHLY otomatik üretim.
- **B2 Hatırlatma / otomatik başlatma / durum geçişi** — `status` enum hazır → `@nestjs/schedule` job + bildirim.
- **B3 Kapak görseli** — `coverImageId` kolon hazır → A1 (CSAM) gelince açılır.
- **B4 Paylaş/derin link + takvim** — `?event=` handler + davet-event bağlama; "Sırala & Görüntüle"/takvim görünümü.

### Track C — Ürün Derinliği (V3 proper)
- **C1 Bildirim sistemi (kalıcı)** — şu an oturum-içi minimal (Sprint 6). Kalıcı `Notification` modeli + mention/DM/etkinlik + okundu + (web push?). En büyük UX boşluğu.
- **C2 Özel emoji** — ertelendi (CSAM A1 kapısı).
- **C3 Forum kanalları** — kanal türü "yakında" disabled → forum işlevi.
- **C4 Video arama** — ses audio-only; video kurul T&S sınırları gerektirir.
...
### PM önerisi (sıralama)
Kısa/additive olduğu için **Track B (etkinlik motoru)** momentum verir + etkinliği tamamlar. Sonra **C1 bildirim** (en büyük UX boşluğu). **Track A** kapalı→açık önizleme önünde duruyor ve dış bağımlılık (CSAM araç + hukuk) uzun sürer → **araştırmasını şimdi paralel başlat**, kod sonra. **Track D** = kurul/strateji turu.
