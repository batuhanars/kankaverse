...
| D22 | **Responsive/mobil uyum yok** — web SPA desktop-first; ~768px altında app-shell (rail 72px + panel 264px **sabit**, drawer/hamburger yok) daralmıyor → telefonda içerik ~75px, kullanılamaz. Landing + auth responsive; app-shell değil. Flutter mobil ertelendiğinden **web = tek mobil yol** | web | responsive durum tespiti / 2026-06-22 | UX (açık kayıtta yüksek — TR trafiği mobil ağırlık); **demo blokeri DEĞİL** (invite-only → mobil rando app'e giremez, responsive landing'i görür) | 🟡 **AKTİF — sözleşme yazıldı 2026-06-23:** `contracts/SPRINT_RESPONSIVE_CONTRACT.md`. Sahip kararı: Discord-masaüstü modeli (panel daraldıkça düşer) + mobilde **tek birleşik sol-drawer** (rail+sidebar). Tailwind v4 `md/lg/xl`=768/1024/1280 belgelenenle birebir + `@vueuse useBreakpoints` hazır → altyapı ucuz. **Fazlı 3 contract:** A (temel `useResponsive`/`useAppShellNav` + app-shell drawer/hamburger + üye paneli overlay≥1280-inline) → B (içerik grid reflow) ∥ C (modal tam-ekran + ayar ekranları). **R7-nötr** (saf görsel). **Ertelendi→V2:** ses/video tile responsive · swipe jesti · dokunmatik sürükle-sırala. **Tetik: açık kayıt öncesi** |
