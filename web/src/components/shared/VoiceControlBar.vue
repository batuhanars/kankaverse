...
<template>
  ...
  <div class="flex items-center justify-center gap-2">
    <!-- Medya tepsisi (Discord-tarzı grup): sustur · sağırlaştır · kamera · ekran -->
    <div class="flex items-center gap-1 p-1 rounded-full" style="background-color: var(--kv-bg-elevated);">
      <!-- Sustur (yalnız konuşma izni varsa) -->
      <button
        v-if="voiceStore.canPublish"
        class="w-10 h-10 flex items-center justify-center rounded-full cursor-pointer transition-colors hover:bg-[var(--kv-accent-subtle)]"
        :title="voiceStore.isMuted ? t('voice.unmute') : t('voice.mute')"
        :style="voiceStore.isMuted ? { backgroundColor: 'var(--kv-danger)', color: '#fff' } : { color: 'var(--kv-text-primary)' }"
      ...
      <button
        ...
        :style="voiceStore.isDeafened ? { backgroundColor: 'var(--kv-danger)', color: '#fff' } : { color: 'var(--kv-text-primary)' }"
      ...
      <button
        ...
        :style="voiceStore.isCameraOn ? { backgroundColor: 'var(--kv-accent-500)', color: '#fff' } : { color: 'var(--kv-text-primary)' }"
      ...
      <button
        ...
        :style="voiceStore.isScreenSharing ? { backgroundColor: 'var(--kv-accent-500)', color: '#fff' } : { color: 'var(--kv-text-primary)' }"
