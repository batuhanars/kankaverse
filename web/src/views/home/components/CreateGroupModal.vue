...
async function create() {
  if (selectedIds.value.size === 0 || creating.value) return
  error.value = ''
  creating.value = true
  try {
    const ids = Array.from(selectedIds.value)
    const name = groupName.value.trim() || undefined
    const channel = await dmStore.createGroup(ids, name)
    emit('created', channel.id)
  } catch {
    // Jenerik hata — GROUP_MINOR_FORBIDDEN veya NOT_FRIEND sebep gösterilmez (T&S: statü sızdırma yok)
    error.value = t('group.createError')
  } finally {
      ...
      <div class="px-5 pb-5 pt-3 shrink-0 border-t" style="border-color: var(--kv-border-subtle);">
        ...
        <div class="flex gap-2 justify-end">
          ...
          <button
            ...
            :class="selectedIds.size === 0 || creating ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:opacity-90'"
            style="background-color: var(--kv-accent-500); color: #fff;"
            :disabled="selectedIds.size === 0 || creating"
            @click="create"
          >
            {{ creating ? t('group.creating') : t('group.createAction') }}
          </button>
        </div>
      </div>
    </div>
