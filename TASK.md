...
- [ ] **SWEEP (sıradaki):** kalan enum kontrolleri → `can(flag)`: kanal/kategori CRUD+reorder (`MANAGE_CHANNELS`, ChannelPanel `isAdmin`), kick/ban (MemberPanel), davet (`CREATE_INVITE`), mesaj sil/pin (`MANAGE_MESSAGES`, MessageArea), genel ayar `MANAGE_GUILD` (adultsOnly OWNER-only kalır). OWNER-only korunur: ortam sil/devret/adultsOnly.
