...
- [ ] **Bilinçli sonraya:** UI yetki-kapısı hâlâ enum (`isOwner||isAdmin`); MANAGE_ROLES-only üyenin ayarları açması + sahip-olmadığın-izni-disable etmek (F1 UX) efektif-izin endpoint'i/DTO gerektirir → ayrı tur (backend zaten F1'i enforce ediyor; güvenlik açığı yok).
