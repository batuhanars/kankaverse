...
- [ ] **LiveKit ses kanalları (audio-only v1)** — V2 en büyük kalem; ayrı altyapı. `.env` hazır. Kurul + sahip onaylı sınırlar (kayıt YOK · video ertelendi · karantina→konuşma kapalı). Sözleşme `contracts/SPRINT_V2_LIVEKIT_CONTRACT.md`; verdikt vault `kurul/verdicts/2026-06-14-livekit-ses-cocuk-guvenligi.md`. **UYGULAMADA.**
  - [ ] Backend: dep (`livekit-server-sdk`) · `GUILD_VOICE` enum + `VoiceSession` + migration · `modules/voice/` (token **R7** · webhook **R7** · participants) · WS olayları · main.ts raw-body
  - [ ] Frontend: dep (`livekit-client`) · ses kanalı türü etkinleştir · katıl/ayrıl/sustur kalıcı bar · katılımcı listesi (WS) · i18n
  - [ ] DoD: iki kullanıcı uçtan uca ses · minör yaş-kapılı sese giremez · kayıt/video yok · testler yeşil
