...
- [x] **Küçük borç temizliği (2026-06-15):**
  ...
  - **rules→description rename → YAPILDI (2026-06-16, sahip talebi):** backend kolon+DTO+servis+tipler + frontend tip/api/store/`draftDescription` + i18n DTO mesajı. Migration **veri-koruyan `RENAME COLUMN`** (drop+add DEĞİL; yerel DB'de 1 dolu değer korundu), `migrate status` temiz. i18n UI etiketleri zaten `guildSettings.description*`. **⚠️ PROD: Railway'de otomatik migrate yok (`start:prod`=düz node) → deploy ile eşzamanlı `prisma migrate deploy` prod DB'ye elle uygulanmalı; yoksa eski/yeni kod ile kolon uyuşmazlığı 500 verir.** Borç kapandı.
